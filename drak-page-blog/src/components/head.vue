<style lang="stylus">
    .head-box
        position: fixed
        top: 0
        left: 0
        right: 0
        transition: all .2s
        background: #fff
        border-bottom: 1px solid #f1f1f1
        color: #909090
        z-index: 250
        height: 60px
    .head-container 
        width: 1000px
        margin: 0 auto
        display: -webkit-box
        display: -ms-flexbox
        display: flex
        align-items: center
        height:60px
    .log 
        margin-right:20px
    .log img
        height: 35px
    .logo
        color: #909399
        font-size: 30px
    nav 
        display: -webkit-box
        display: -ms-flexbox
        display: flex
        align-items: center
    
    nav a 
        padding: 0 15px
        font-size: 15px
        color: #8590a6
        cursor: pointer
    nav a:hover
        color:#007fff
    .search
        display: -webkit-box
        display: -ms-flexbox
        display: flex
        margin-left:20px
    .search-bar
        width: 300px
        background:#f6f6f6
        border: 1px solid #ebebeb
        border-radius: 3px
        padding: 4px 10px
        box-sizing: border-box
        transition: all .2s ease
    .search-bar input 
        width: 100%
        font-size: 14px
        line-height: 24px
        background: transparent
        color:#8590a6
    .focus
        width: 380px
        background:#fff
        border:1px solid #8590a6
    .ask-question
        color: #fff
        background-color: #0084ff
        padding: 0 14px
        margin-left: 16px
        transition: all .3s ease
        line-height: 30px
        font-size:16px
        border-radius:5px
    .isHide
        opacity:0
    .operate 
        display: -webkit-box
        display: -ms-flexbox
        display: flex
        flex:1 auto
        justify-content: flex-end
    .operate  li
        padding: 0 5px
        color: #007fff
        cursor: pointer
        line-height 30px
    .operate .write
        padding: 0 20px
    .publishBtn
        border-color: #0080ff
        color: #0080ff
    .head 
        width: 35px
        height: 35px
        
    .head img
        max-width: 100%
        // border-radius: 20px
    .head-list>li
        padding: 10px 10px
        cursor: pointer
        color:#8590a6
    .head-list>li svg 
        margin-right: 6px
        vertical-align: middle
    .head-list>li:hover
        background-color: #f1f1f1
    .head .el-popover 
        padding: 0;
</style>
<template>
    <header class="head-box">
        <div class="head-container">
            <div class="log logo">
                <!-- <img src="http://ozpmvfr0u.bkt.clouddn.com/resources/0/201801191439408723.png" alt=""> -->
                LS
            </div>
            <nav>
                <router-link to="/">首页</router-link>
                <a href="https://github.com/linsudeer" target="_blank">开源库</a>
                <!-- <router-link to="/Activity">活动</router-link> -->
            </nav>
            <div class="search">
                <div class="search-bar" v-bind:class="{ focus:isFocus }">
                    <input name="search" placeholder="请输入你要搜索的内容" @focus="focusInput()" 
                    @blur="blurInput()">
                </div>
                <button class="ask-question" v-bind:class="{ isHide:isFocus }">提问</button>
            </div>
            <ul v-if="isLogin" class="operate">
                <!-- <li class="write"><router-link to="/write">写文章</router-link></li> -->
                <li>
                    <router-link to="/write" tag="a" target="_blank">
                        <el-button size="small" class="publishBtn" icon="el-icon-edit-outline">写文章</el-button> 
                    </router-link>
                </li>
                <!-- <li>{{user.userNickname}}</li> -->
                <li class="head">
                    <el-popover placement="bottom" width="50" trigger="click">
                        <img src="../../static/img/head1.png" slot="reference">
                        <ul class="head-list">
                            <li>
                                <svg fill="currentColor" viewBox="0 0 24 24" width="16" height="16"><path d="M15.417 12.923c-.376.653-.837 1.281-.763 1.863.292 2.273 5.562 1.77 6.78 3.048.566.595.566.664.566 4.164-6.611-.07-13.363 0-20 0 .027-3.5 0-3.478.62-4.164 1.303-1.44 6.581-.715 6.78-3.133.045-.545-.38-1.114-.763-1.778C6.511 9.233 5.697 2 12 2s5.422 7.443 3.417 10.923z" fill-rule="evenodd"></path></svg>我的主页</li>
                            <li><svg fill="currentColor" viewBox="0 0 24 24" width="16" height="16"><path d="M20.868 17.185a.896.896 0 0 1-.452.137c-.123 0-1.397-.26-1.617-.233-1.354.014-1.78 1.276-1.835 1.742-.055.453 0 .892.191 1.303a.8.8 0 0 1-.068.851C16.224 21.877 14.922 22 14.73 22a.548.548 0 0 1-.356-.151c-.11-.096-.685-1.138-1.069-1.468-1.304-.955-2.247-.329-2.63 0-.398.33-.672.7-.836 1.125a.632.632 0 0 1-.329.37c-1.354.426-2.918-.919-3.014-1.056a.564.564 0 0 1-.123-.356c-.014-.138.383-1.276.342-1.688-.342-1.9-1.836-1.687-2.096-1.673a3.192 3.192 0 0 0-.918.178.873.873 0 0 1-.59-.055c-.887-.462-1.136-2.332-1.109-2.51.055-.315.192-.521.438-.604.425-.164.809-.452 1.151-.85.931-1.262.343-2.25 0-2.634-.342-.356-.726-.645-1.15-.809-.138-.041-.234-.151-.33-.316-.38-1.434.613-2.552.867-2.77.255-.22.6-.055.723 0 .425.164.877.219 1.343.15C6.7 6.636 6.784 5.141 6.81 4.908c.014-.247-.11-1.29-.137-1.4a.488.488 0 0 1 .027-.315C7.317 2.178 9.071 2 9.222 2a.56.56 0 0 1 .439.178c.11.124.63 1.111 1 1.4.4.338 1.583.83 2.59.013.397-.274.959-1.29 1.082-1.413A.55.55 0 0 1 14.717 2c1.56 0 2.329 1.029 2.438 1.22a.458.458 0 0 1 .069.371c-.028.151-.329 1.152-.26 1.605.365 1.537 1.383 1.742 1.89 1.783.493.028 1.644-.356 1.809-.343a.63.63 0 0 1 .424.206c.535.31.85 1.715.905 2.14.027.233-.014.439-.11.562-.11.138-1.165.714-1.48 1.112-.855.982-.342 2.25-.068 2.606.26.37 1.22.905 1.288.96.15.137.26.302.315.494.146 1.413-.89 2.387-1.069 2.47zm-8.905-.535c.644 0 1.246-.123 1.822-.356a4.576 4.576 0 0 0 1.493-1.016 4.694 4.694 0 0 0 1-1.495c.247-.562.357-1.18.357-1.81 0-.659-.11-1.262-.356-1.825a4.79 4.79 0 0 0-1-1.481 4.542 4.542 0 0 0-1.494-1.002 4.796 4.796 0 0 0-3.631 0 4.627 4.627 0 0 0-1.48 1.002c-.424.425-.767.919-1 1.481a4.479 4.479 0 0 0-.37 1.825c0 .644.124 1.248.37 1.81a4.62 4.62 0 0 0 1 1.495c.425.426.918.768 1.48 1.016a4.677 4.677 0 0 0 1.809.356z" fill-rule="evenodd"></path></svg>设置</li>
                            <li @click="logout"><svg fill="currentColor" viewBox="0 0 24 24" width="16" height="16"><path d="M2 11.999c0-2.756 1.154-5.417 3.167-7.3a1.266 1.266 0 0 1 1.73 1.847 7.396 7.396 0 0 0-2.367 5.453c0 4.119 3.35 7.47 7.47 7.47 4.119 0 7.47-3.351 7.47-7.47a7.41 7.41 0 0 0-2.279-5.37 1.266 1.266 0 0 1 1.76-1.819A9.923 9.923 0 0 1 22 12c0 5.513-4.486 10-10 10s-10-4.487-10-10zm8.699-.482V3.26a1.26 1.26 0 1 1 2.52 0v8.257a1.26 1.26 0 1 1-2.52 0z" fill-rule="evenodd"></path></svg>退出</li>
                        </ul>
                    </el-popover>
                </li>
            </ul> 
            <ul v-else class="operate">
                <li @click="login()">登录</li>
                <li @click="register()">注册</li>
            </ul> 
        </div>
        <masks :regOrLogin="operateType" :name="name" v-show="showMask" v-on:sendMsgToHideMask="sendMsgToHideMask" v-on:changeLoginHeader="changeLoginHeader"></masks>
    </header>
</template>

<script>
import LoginApi from '@/api/user'
import Masks from '@/components/mask'
import Store from 'store'
import Global from '@/global'
export default {
    name:'com-header',
    components: {
      Masks
    },
    data() {
        return {
            isFocus:false,
            showMask:false,
            name:'',
            operateType:"",
            isLogin:Store.get(Global.ISLOGIN),
            user:Store.get(Global.USER)
            
        }
    },
    computed: {
        
    },
    mounted: function () {
        let _this = this;
        
    },
    methods: {
        focusInput:function(){
            this.isFocus = true;
        },
        blurInput:function(){
            this.isFocus = false;
        },
        login:function(){
            this.showMask = true;
            this.operateType = "Login",
            this.name = "登录"
        },
        register:function(){
            this.showMask = true;
            this.operateType = "Register"
            this.name = "注册"
        },
        sendMsgToHideMask:function(){
            this.showMask = false;
        },
        changeLoginHeader: function (user){
            this.showMask = false;
            this.user = user;
            this.isLogin = true;
        },
        toWrite: function(){
                this.$router.push('/write/')
        },
        logout: function() {
            let _this = this;
            LoginApi.logout().then(function(res){
                Store.set(Global.ISLOGIN, false);
                _this.isLogin = false;
                Store.remove(Global.USER);
            })
        }
    }
}

</script>
